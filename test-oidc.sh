...
TOKEN_RESPONSE=$(curl -s -X POST http://localhost:3000/token \
  ...
  -d "grant_type=password&username=${TEST_USER_EMAIL:-6mzhptd20p@ozsaip.com}&password=${TEST_USER_PASSWORD:-0Uxbw7}&client_id=test-client&scope=openid%20profile%20email")
...
MISSING_RESPONSE=$(curl -s -X POST http://localhost:3000/token \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=password&username=${TEST_USER_EMAIL:-6mzhptd20p@ozsaip.com}&password=${TEST_USER_PASSWORD:-0Uxbw7}")
...
echo ""
print_status "👤 Usuario de prueba: ${TEST_USER_EMAIL:-6mzhptd20p@ozsaip.com}"
print_status "🔑 Contraseña: ${TEST_USER_PASSWORD:-0Uxbw7}"
echo ""
